...
def create_mock_role(role_id: int, name: str, **kwargs) -> MagicMock:
    """Helper to create a properly mocked Discord role."""
    ...
    role.is_premium_subscriber = MagicMock(return_value=kwargs.get("is_premium_subscriber", False))
...
class TestRoleManagementLogic:
    ...
    def test_get_manageable_roles_filters_normal_roles(self):
        ...
        premium_role = create_mock_role(300, "Nitro Booster", is_premium_subscriber=True)
        integration_role = create_mock_role(400, "YouTube Subscriber", is_integration=True)
...
class TestUnjailRoleRestoration:
    ...
    async def test_unjail_after_role_changes(self, db_service):
        """
        Test that stored roles remain consistent even if user's roles changed
        after jailing (edge case where roles were manually modified).
        """
        case_controller = CaseController(db_service)
