...
class TestEventHandler(unittest.TestCase):
    ...
    def test_load_events(self):
        ...
        with unittest.mock.patch('os.listdir',
                                 return_value=['on_leave.py',
                                               'on_join.py']):
            self.assertEqual(self.event_handler.load_events(), None)

        # Ensure that self.bot.load_extension is called for each event module
        self.mock_bot.load_extension.assert_called_with('tux.tux_events.events.on_leave')
    ...
    def test_on_ready(self):
        ...
        with unittest.mock.patch('logging.info') as mock_info:
            self.assertEqual(self.event_handler.on_ready(), None)  # It should return None
    ...
    def test_setup(self):
        ...
        with unittest.mock.patch.object(self.mock_bot,
                                        'add_cog') as mock_add_cog:
            self.assertEqual(self.event_handler.setup(self.mock_bot,
                                                      debug=True), None)
