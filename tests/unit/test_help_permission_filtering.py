...
class TestHelpPermissionFiltering:
    ...
    async def test_can_run_command_permission_system(
        ...
        mock_perm_system.get_command_permission = AsyncMock(return_value=None)  # Unconfigured
        mock_perm_system.get_user_permission_rank = AsyncMock(return_value=0)  # No rank

        with patch("tux.help.data.get_permission_system", return_value=mock_perm_system), \
