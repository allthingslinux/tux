...
from tux.cogs.levels.level import Level
...
def level_cog(mock_bot_with_container):
    ...
    with patch('tux.cogs.levels.level.generate_usage'):
        with patch('tux.cogs.levels.level.LevelsService') as mock_levels_service:
...
class TestLevelCog:
    ...
    async def test_level_command_self(self, level_cog):
        ...
        with patch('tux.cogs.levels.level.CONFIG') as mock_config:
    ...
    async def test_database_service_fallback(self, mock_bot_with_container):
        ...
        with patch('tux.cogs.levels.level.generate_usage'):
            with patch('tux.cogs.levels.level.LevelsService'):
                cog = Level(mock_bot_with_container)
