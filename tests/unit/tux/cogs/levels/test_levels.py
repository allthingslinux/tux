...
from tux.cogs.levels.levels import Levels
...
def levels_cog(mock_bot_with_container):
    ...
    with patch('tux.cogs.levels.levels.generate_usage'):
        with patch('tux.cogs.levels.levels.LevelsService') as mock_levels_service:
...
class TestLevelsCog:
    ...
    async def test_database_service_fallback(self, mock_bot_with_container):
        ...
        with patch('tux.cogs.levels.levels.generate_usage'):
            with patch('tux.cogs.levels.levels.LevelsService'):
                cog = Levels(mock_bot_with_container)
