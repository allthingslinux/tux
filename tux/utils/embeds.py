...
#         embed.color = CONST.COLORS[state]

#         embed.set_author(
#             name=state.capitalize() if state else "Info",
#             icon_url=CONST.ICONS[state] if state else CONST.ICONS["DEFAULT"],
#         )
...
#         embed.set_footer(
#             text=f"{user.name}@atl $ ∕tux {latency}",
...
#         embed.color = kwargs.get("color", discord.Color.blurple())
...
#         footer: tuple[str, str | None] = EmbedCreator._get_footer(ctx, interaction)
#         embed.set_footer(text=footer[0], icon_url=footer[1])
...
#     @staticmethod
#     def add_field(embed: discord.Embed, name: str, value: str, inline: bool = False) -> None:
#         embed.add_field(name=name, value=value, inline=inline)
