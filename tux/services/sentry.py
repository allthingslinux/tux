...
def map_db_operation_to_type(op: str) -> str:
    """Map controller operation name to standardized type (read/create/update/delete/count/other)."""
    ...
    return next((mapped for prefix, mapped in mapping.items() if prefix in op), "db.other")


def map_transaction_name_to_type(name: str) -> str:
    """Map controller transaction name to standardized transaction op strings."""
    if not name.startswith("db.controller."):
        return name
    mapping = {
    ...
    return next((mapped for prefix, mapped in mapping.items() if prefix in name), "db.controller.other_operation")
