...
class LevelsController:
    ...
    async def increment_xp(self, user_id: int, guild_id: int, member: discord.Member, guild: discord.Guild) -> None:
        ...
        try:
            ...
            if await self.on_cooldown(user_id, guild_id):
                await db.levels.update(
                    where={"user_id_guild_id": {"user_id": user_id, "guild_id": guild_id}},
            ...
            if await self.blacklisted(user_id, guild_id):
                return
    ...
    async def on_cooldown(self, user_id: int, guild_id: int) -> bool:
        """
        Check if a user is on cooldown for XP.
        ...
        if time_between_messages < cooldown_period:
            return True
        return False

    async def blacklisted(self, user_id: int, guild_id: int) -> bool:
        """
        Get the level for a user in a guild.
