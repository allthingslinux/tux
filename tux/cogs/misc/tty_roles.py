...
class TtyRoles(commands.Cog):
    ...
    @staticmethod
    async def try_create_role(member: discord.Member, role_name: str):
        try:
            ...
            logger.error("Creating the tty role failed.")
        return None

    @staticmethod
    async def try_assign_role(member: discord.Member, role: discord.Role):
        try:
