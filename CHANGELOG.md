...
* **Snippets**: `$lsf` command (aliases `listsnippetsfrom`, `lsb`) lists snippets created by a specific member, with an optional search query; uses `FlexibleUserConverter` for mentions, user IDs, and usernames including users who have left the guild (`$snippets`/`$ls` remains the general list with optional query only)
...
* **Wiki**: Enhanced language code matching to only include known translation markers
...
* **Case creation**: Prevented manual ID assignment during case creation
