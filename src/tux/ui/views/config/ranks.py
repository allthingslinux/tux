...
async def initialize_default_ranks(db: DatabaseCoordinator, guild_id: int) -> None:
    ...
    """
    logger.info(f"initialize_default_ranks called for guild {guild_id}")
    ...
    if not guild_record:
        ...
        try:
            ...
            logger.info(f"Successfully registered guild {guild_id}")
    ...
    try:
        existing_ranks = await db.permission_ranks.get_permission_ranks_by_guild(guild_id)
        logger.debug(f"Found {len(existing_ranks)} existing ranks")
        if existing_ranks:
    ...
    for rank, data in DEFAULT_RANKS.items():
        logger.debug(f"Creating rank {rank} with data: {data}")
        try:
    ...
    logger.info(f"Successfully created all default ranks for guild {guild_id}")
