...
class GuildConfigController(BaseController):
	...
	# Log getters
	@with_session
	async def get_report_log_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else cfg.report_log_id

	@with_session
	async def get_audit_log_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else cfg.audit_log_id

	@with_session
	async def get_mod_log_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else cfg.mod_log_id

	@with_session
	async def get_join_log_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else cfg.join_log_id

	@with_session
	async def get_private_log_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else cfg.private_log_id

	@with_session
	async def get_dev_log_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
	...
	@with_session
	async def get_jail_channel_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else cfg.jail_channel_id

	@with_session
	async def get_starboard_channel_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else cfg.starboard_channel_id

	@with_session
	async def get_general_channel_id(self, guild_id: int, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
	...
	# Perm-levels
	@with_session
	async def get_perm_level_role(self, guild_id: int, perm_field: str, *, session: AsyncSession) -> Optional[int]:
		cfg = await session.get(GuildConfig, guild_id)
		return None if cfg is None else getattr(cfg, perm_field)

	@with_session
	async def update_perm_level_role(self, guild_id: int, perm_level: str, role_id: int, *, session: AsyncSession) -> None:
