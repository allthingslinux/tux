...
class StarboardController(BaseController):
    ...
    @with_session
    async def get_starboard_by_guild_id(self, guild_id: int, *, session: Any = None) -> Starboard | None:
        return await session.get(Starboard, guild_id)


class StarboardMessageController(BaseController):
    @with_session
    async def get_starboard_message_by_id(self, message_id: int, *, session: Any = None) -> StarboardMessage | None:
