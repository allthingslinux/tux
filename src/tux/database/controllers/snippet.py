...
class SnippetController(BaseController):
    ...
    async def get_snippet_by_name_and_guild_id(
        ...
        stmt = select(Snippet).where(
            and_(Snippet.guild_id == guild_id, Snippet.snippet_name.ilike(snippet_name))
        )
        res = await session.execute(stmt)
    ...
    async def get_all_aliases(self, snippet_name: str, guild_id: int, *, session: AsyncSession) -> List[Snippet]:
        stmt = select(Snippet).where(and_(Snippet.alias == snippet_name, Snippet.guild_id == guild_id))
        res = await session.execute(stmt)
        return list(res.scalars())